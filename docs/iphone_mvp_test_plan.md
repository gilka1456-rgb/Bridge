...
| P0-5 | Save world map | Tap save after mapping is mapped/extending and the preview anchor is ready. | Save is blocked until the preview anchor appears in the current ARFrame, then succeeds and placement appears under My Placements. Diagnostics record preview-anchor readiness, the WorldMap filename, anchor count, file size, mapping status, location availability, heading, and location/heading provider summary. | |
