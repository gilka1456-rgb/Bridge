...
| P1-10 | Invalid local data | Delete a placement/avatar or use Diagnostics to confirm missing WorldMap, invalid transform, or WorldMap-without-expected-anchor cases after repeated tests. | Discover explains when candidates are skipped because the avatar, `.worldmap` file, or expected placement anchor is missing; Diagnostics exposes invalid transform counts and cleanup can remove bad placement anchors. This is not counted as AR relocalization failure. | |
