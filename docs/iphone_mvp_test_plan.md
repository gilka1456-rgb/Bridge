...
| P1-6 | AR interruption | During Scan, Place, and Discover, briefly background the app or trigger a system interruption, then return. | App records interruption/recovery in `诊断`; Place clears the preview and requires a fresh plane tap before save; Scan asks to reacquire the body; Discover restarts matching instead of keeping stale relocalized state. | |
