...
| P1-6 | AR interruption | During Scan, Place, and Discover, briefly background the app or trigger a system interruption, then return; also switch away from Discover after it renders a placement and come back. | App records interruption/recovery in `诊断`; Scan clears cached body/frame data and restarts Body Tracking; Place clears the preview and requires a fresh plane tap before save; Discover clears rendered avatars/cards on interruption or view exit and restarts matching instead of keeping stale relocalized state. | |
