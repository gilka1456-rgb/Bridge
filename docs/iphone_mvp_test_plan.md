...
| P0-5 | Save world map | Tap save after mapping is mapped/extending and the preview anchor is ready; try tapping save repeatedly while it is saving. | Save is blocked until the preview anchor appears in the current ARFrame, duplicate save requests are ignored, then one placement appears under My Placements. Diagnostics record preview-anchor readiness, the WorldMap filename, anchor count, file size, mapping status, location availability, heading, and location/heading provider summary. If saving fails, the error must include the AR mapping status, current WorldMap anchor count, and the expected preview anchor ID when the map is missing that anchor. If local placement JSON writing fails after WorldMap creation, the in-memory placement is rolled back, the new WorldMap is cleaned up, and the preview remains available for retry. | |
| P0-6 | Leave app | Switch tab, lock the phone or background the app while Scan, Place, and Discover are active, then return. Also close and reopen once after saving. | No crash. Active AR pages pause when the app becomes inactive/backgrounded and resume only when returning to that same visible AR page. Scan requires a fresh body frame, Place requires a fresh plane tap if an unsaved preview existed, and Discover rematches WorldMap instead of keeping stale rendered content. Diagnostics contain the app background/foreground events for the active AR page. | |
...
| P0-8 | Hit test | Tap the visible avatar, then tap `留存`. Also confirm `留存` is unavailable before any avatar is rendered. Restart matching after one successful 留存 and confirm the old preview is not reused. | Correct placement card opens with the saved message and comment thread. `诊断` records the tapped placement ID, WorldMap filename, and message preview; the rendered placement ID summary and Diagnostics placement reference show the same placement/message context. The `留存` action is gated until relocalized content is rendered, then shows a shareable preview and records whether the ARView snapshot succeeded, the active/rendered WorldMap, rendered placement count, snapshot pixel size, and any share-file generation failure reason. Relocalization resets clear stale retention previews. | |
| P0-9 | Comment | Add top-level comment, reply, reaction, and like. Reopen placement. | Engagement persists locally and comment actions are visible in `诊断` recent events. The visible two-level comment list and engagement count agree after restart; orphaned or over-nested replies from stale local JSON are cleaned. If a comment or placement was deleted before a reaction/like tap is processed, or a local comment JSON write fails, the app records a failed/warning event instead of a misleading durable-success event. Failed new comment/reply writes are rolled back from memory and keep the typed input available for retry; failed reaction/like writes roll back the visible toggle state. | |
| P0-10 | Delete | Delete a placement from My Placements. Reopen Discover. Also delete one comment and one avatar if test data allows. | Deleted placement and related comments no longer appear. If Discover was holding a rendered avatar or open card for deleted local data, it clears stale state and rematches instead of keeping the old avatar/card visible. `诊断` records the deleted placement/avatar/comment ID, WorldMap cleanup summary, removed comment count, stale Discover cleanup when applicable, and any local deletion persistence warning if JSON writing fails. Failed placement/avatar deletion writes roll back visible deletion state and skip WorldMap cleanup instead of leaving restored records pointing at deleted maps; avatar detail stays open when that rollback happens. Failed comment deletion writes roll back the visible deletion state instead of pretending the comment is gone until restart. | |
...
| P1-3a | Empty Discover | On a clean install or after deleting all local placements, open Discover. | The HUD shows there are no placements and the bottom prompt asks the tester to create the first placement. Diagnostics records that WorldMap relocalization was skipped because there are no local placements; the app must not report this as a relocalization failure. | |
| P1-4 | App restart | Force quit, reopen, and Discover at original spot. Also check Diagnostics immediately after any save/comment action unexpectedly fails or disappears. | Local avatar/placement/comment data loads; relocalization can still be attempted. If a local JSON file cannot decode after restart or cannot be written during the test session, the Diagnostics tab and exported report show a load/save warning instead of silently hiding the cause. If a referenced `.worldmap` file exists but cannot decode, Diagnostics and Discover errors call out the decode failure as a relocalization blocker instead of reporting it like a save failure. | |
| P1-5 | Poor light | Repeat Discover with reduced light. | Failure is graceful; app does not render stale avatars as success. | |
| P1-6 | AR interruption, app backgrounding, and ARSession failure | During Scan, Place, and Discover, briefly lock/background the app or trigger a system interruption, then return; if Xcode/device logs surface an ARSession failure, keep the app open and check the active screen state; also switch away from Discover after it renders a placement and come back. | App records scenePhase background/foreground, interruption/recovery, or ARSession failure cleanup in `诊断`; Scan clears cached body/frame data and restarts Body Tracking after interruption or foreground return; Place clears the preview and requires a fresh plane tap before save; Discover clears rendered avatars/cards on interruption, ARSession failure, foreground rematch, or view exit and restarts matching instead of keeping stale relocalized state. | |
...
| P1-12 | Invalid placement cleanup | Create or simulate an invalid placement, open `诊断`, and tap `清理无效放置`; also delete placements/avatars whose WorldMap is either unreferenced or still referenced by another placement. | Only placements missing avatar data, missing `.worldmap` files, invalid WorldMap filenames, WorldMaps that fail to decode, decoded WorldMaps missing the placement's expected anchor, or invalid transform data are removed. Diagnostics summarize invalid WorldMap filename counts separately from missing files and report `WorldMap 解码失败` separately from `WorldMap 缺少目标锚点`; deletion diagnostics show this operation's WorldMap cleanup result, including deleted/missing/still-referenced counts, without reusing an older maintenance summary. If local JSON writing fails, invalid placement cleanup rolls back the visible deletion state and skips WorldMap cleanup instead of leaving restored records with partially removed engagement or maps. | |
...
| P1-14 | Orphaned comment cleanup | Delete a placement or avatar with comments, force quit, reopen, and export `诊断`. Also delete a top-level comment that has replies after repeated local tests. | Comments/reactions/likes for deleted placements do not reappear after reload, deleting a comment removes its full reply tree, and `诊断` records the deletion action plus removed placement/comment counts. If restart-time orphan cleanup cannot persist, Diagnostics warns that orphaned comments may return after another restart. | |
...
- Whether `诊断` contains the scenePhase background/foreground event for the AR page that was visible during lock screen or app switch.
...
- Whether each placement's Diagnostics row shows `anchorInWorldMap`, especially for decoded WorldMaps that are missing the expected placement anchor.
