...
| P1-12 | Invalid placement cleanup | Create or simulate an invalid placement, open `诊断`, and tap `清理无效放置`; also delete placements/avatars whose WorldMap is either unreferenced or still referenced by another placement. | Only placements missing avatar data, missing `.worldmap` files, invalid WorldMap filenames, decoded WorldMaps missing the placement's expected anchor, or invalid transform data are removed. Diagnostics summarize invalid WorldMap filename counts separately from missing files and report `WorldMap 缺少目标锚点`; deletion diagnostics show this operation's WorldMap cleanup result, including deleted/missing/still-referenced counts, without reusing an older maintenance summary. If local JSON writing fails, invalid placement cleanup rolls back the visible deletion state and skips WorldMap cleanup instead of leaving restored records with partially removed engagement or maps. | |
