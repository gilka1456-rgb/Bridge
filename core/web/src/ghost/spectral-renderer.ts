...
export const SPECTRAL_CYBER_MATRIX_RESPONSE = Object.freeze({
  ...
  surfaceLift: 0.12,
  emission: 0.13,
  appearanceContourEmission: 0.26,
  recessedFoldShadow: 0.28,
});
...
const spectralSurfaceFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    cyberColor *= 0.76 + capturedSignalTone * 0.48;
...
const fantasyParticleVertexShader = /* glsl */ `
  ...
  void main() {
    ...
    float tipAttachment = pow(clamp(wispUv.y, 0.0, 1.0), 1.18);
    vec3 wispAnchor = posedPosition
      + surfaceUp * surfaceRise * tipAttachment
      + surfaceSide * surfaceSway * tipAttachment
      + posedNormal * mix(0.008, normalDrift, 0.35 + tipAttachment * 0.65);
...
const fantasyParticleFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    float baseFade = mix(0.22, 1.0, smoothstep(0.0, 0.14, height));
