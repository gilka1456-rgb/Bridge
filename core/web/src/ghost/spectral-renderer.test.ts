...
describe("Spectral Render V3 core", () => {
  ...
  it("adds deterministic V5 fantasy palettes and tiered GPU wisps without changing the body passes", () => {
    ...
    expect(fantasySurface.fragmentShader).toContain("fantasySoot");
    expect(fantasySurface.fragmentShader).toContain("fantasySootField");
    expect(fantasySurface.fragmentShader).toContain("fantasyEmberCrack");
    ...
    expect(SPECTRAL_FANTASY_SOUL_RESPONSE.darkCoreMaximum).toBeLessThan(0.68);
    expect(SPECTRAL_FANTASY_SOUL_RESPONSE.strataEmission).toBeGreaterThan(0.2);
    expect(SPECTRAL_FANTASY_SOUL_RESPONSE.crustExtinction).toBeGreaterThan(0.35);
    expect(SPECTRAL_FANTASY_SOUL_RESPONSE.emberCrackEmission).toBeGreaterThan(0.1);
    ...
    expect((highParticles.material as THREE.ShaderMaterial).vertexShader).toContain("ribbonHeight");
    ...
    expect((highParticles.material as THREE.ShaderMaterial).fragmentShader).toContain("centerWander");
    expect((highParticles.material as THREE.ShaderMaterial).fragmentShader).toContain("plumeBody");
    expect((highParticles.material as THREE.ShaderMaterial).fragmentShader).not.toContain("ribbonBody");
  ...
  it("samples surface effects deterministically by triangle area with normalized four-bone weights", () => {
    ...
    const firstGeometry = (first.getObjectByName("spectral-v5-fantasy-particles") as THREE.Mesh).geometry;
    const secondGeometry = (second.getObjectByName("spectral-v5-fantasy-particles") as THREE.Mesh).geometry;
    ...
    expect(sampledPositions.count).toBe(100 * 4);
    for (let sample = 0; sample < sampledPositions.count; sample += 4) {
  ...
  it("redistributes fantasy particles and cyber signals away from the distal hand", () => {
    ...
    const effectGeometries = [
      (fantasy.getObjectByName("spectral-v5-fantasy-particles") as THREE.Mesh).geometry,
    ...
    for (const geometry of effectGeometries) {
      ...
      const expansion = geometry.userData.spectralWispAnchorCount ? 4 : 1;
      expect(sampledRegions.count).toBe(64 * expansion);
  ...
  it("keeps fantasy particles in a rising surface flow and cyber glyphs locked to the projection", () => {
    ...
    expect(Math.max(...fantasySamples.map((sample) => sample.normalOffsetMeters))).toBeGreaterThan(0.03);
  ...
  it("adds deterministic V6 projection styling within the tiered draw budget", () => {
    ...
    expect(material.fragmentShader).toContain("appearanceContour");
