...
6. V5.16 奇幻灵体以《黑暗之魂 3》／《艾尔登法环》的灵体视觉语言为方向：完整、致密的外表负责人体结构，能量染色、贴附表面的魂流、灰烬斑驳、柔和轮廓光和稀疏逸散颗粒负责灵体感，不再依靠透视人体内部。主表面遮挡提高到接近实体，并把扫描明暗、缓慢魂蚀和灰烬层叠加在最外层；背景不会从人体中心穿透，灵体感集中在表面与轮廓。原先位于表面内侧且关闭深度测试的魂流层已移到外表并重新启用深度测试。容易强化几何分段感的水平亮带已改为沿身体表面纵向上升的扭曲魂流，并用低频灰烬壳、细颗粒和受限法线扰动打破平滑塑料感，轮廓仍保持完整稳定。V5.16 进一步建立“深色实体内核—同色表面魂流—高亮边缘雾焰”三层材质，主体遮挡下限提高到 94%，表面与外壳的火焰相位完全使用全身归一化坐标，经过胸、肩、手、腿时不会重新起算或暴露几何分区。新扫描还会从四向照片提取去颜色、去曝光和模糊化后的明暗场，以朝向权重烘焙到共享人体顶点；它只保留衣褶和宽尺度体表层次，不保存清晰照片纹理。每向压缩为 64×128、4 bit，四向合计约 20 KB；旧虚像自动使用中性明暗，不受影响。轮廓层沿局部表面法线做毫米级推出，120/48/0 分档魂屑与贴地魂雾继续保留。颜色不再绑定红灵／白灵预设，用户可选择任意主色，系统自动派生同色系暗部、亮边、雾焰和粒子色；白色与灰色会保持中性，不再被自动配色染出暖色暗部。
7. V6.11 赛博人物投影保留完整人体前表面、细扫描载波、地面投影源、96/40/0 分档空间信号符号，以及每 3.2 秒一次、持续 120ms 且可完全恢复的局部相位事件。主表面同样建立遮蔽下限，不显示人体内部；原先覆盖全身的等距横纹已减弱并加入空间扭曲，地面核心与像素阵列驱动自下而上的重建波，脚部信号更稳定、向上逐渐出现数据列完整度变化，轮廓只保留克制的青／辅色色散。宽数据块、高频信号噪声和完整度变化均由时间插值哈希连续过渡，不再按离散帧整片跳变；细扫描载波也改为全身连续坐标，不会在身体区域边界重置相位。用户自选主色会自动派生对比辅色，投影源与人物保持一致。
