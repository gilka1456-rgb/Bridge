...
6. V5.10 红灵/白灵共用同一奇幻 shader：体内能量、主轮廓壳与低密度外层光晕分工渲染；主表面使用视线厚度吸收代替实体式方向高光，厚处沉入阴影、薄处透出魂光，并以厘米级以内的连续轮廓呼吸和稳定雾化噪声打散规则外缘。轮廓层不再把整个人体从原点同比放大，而是沿局部表面法线做 8–26mm 的分层推出，因此手、脚、肩与头部不会出现比例相关的复制壳错位。向上流动的内层魂流与连续孔隙进一步削弱透明塑料感，同时保留完整人体轮廓。120/48/0 分档 GPU 魂屑改为稀疏的纵向上升细迹，不再形成密集圆点；高／中画质另有无硬圆环的贴地魂雾，使脚部与场景形成低密度能量交换。白灵另有正常混合的深色对比轮廓，在明亮场景中也不会融入背景。
7. V6.7 赛博人物投影已实现青／紫双预设、细扫描带、数据块、地面投影源、96/40/0 分档空间信号符号，以及每 3.2 秒一次、持续 120ms 且可完全恢复的局部相位事件；最高画质增加独立相位余像层，在平时保留微弱载波断层、相位事件中生成短距离色散错位。赛博轮廓与相位余像同样改为毫米级法线偏移，避免近景出现同比放大的人体复制层。投影源作为地面与人体之间的独立 decal 渲染，不再被预览地板遮挡，身体周围的十字数据包用于表现低信号传输。
8. 两种风格共享同一人体、骨骼、深度、LOD、质量架构和地面交互接口；奇幻分支提供贴地魂雾，赛博分支提供投影阵列，后续风格无需改动人体建模即可接入自己的场景交互。V10 共享人体把头部纵向比例、男性腰胯过渡和双腿轴线收敛到成人范围，并用横向形成掌宽、纵深持续收窄的各向异性手部体元替换旧版对称鼓包手套，使肩、腰、胯、手腕和手掌不再呈现低模假人比例；手部仍保持单段连续场，因此最低 LOD 不会丢失指端或缩短手臂。抬臂姿势改用全身统一的四骨骼线性混合，并在连续网格上预先平滑权重；CPU 回退与 GPU 渲染共用相同矩阵和权重，胸口到肘部的三角拉伸已明显收敛，极端举手接缝另有自动拉伸上限回归测试。目标风格的人物浮动已从旧版 ±4cm 收敛到 ±6mm，地面交互会反向抵消人物微动并始终锁定在地板高度，不再出现底座悬浮。四张全身测试图已全部通过识别（单图轮廓质量 91%–97%，重建质量 90%）。姿势以标准骨盆为根节点，照片取景偏移不会再平移或压低人物；局部缺脚的正面帧会自动让位给完整背面帧，不再破坏躯干和双腿。
