...
- 默认旧路径仍使用半透明主层与柔光外壳；`ghost-render-v3=1` 改用结构深度预写、预乘透明主表面和受限加色背壳，最终风格特效在 V5/V6 完成。
...
5. 下一步是 V4 移动端 LOD 与质量控制，随后在 V5/V6 分别完成奇幻灵体和赛博投影的最终风格。
6. 持续保持扫描、媒体生命周期、本地数据迁移和旧渲染回退测试完整。
