...
8. 两种风格共享同一人体、骨骼、深度、LOD、质量架构和地面交互接口；奇幻分支提供贴地魂雾，赛博分支提供投影阵列，后续风格无需改动人体建模即可接入自己的场景交互。目标风格的人物浮动已从旧版 ±4cm 收敛到 ±6mm，地面交互会反向抵消人物微动并始终锁定在地板高度，不再出现底座悬浮。四张全身测试图已全部通过识别（单图轮廓质量 91%–97%，重建质量 90%）。姿势以标准骨盆为根节点，照片取景偏移不会再平移或压低人物；局部缺脚的正面帧会自动让位给完整背面帧，不再破坏躯干和双腿。
