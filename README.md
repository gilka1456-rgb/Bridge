...
6. V5.11 红灵/白灵共用同一奇幻 shader：体内能量、主轮廓壳与低密度外层光晕分工渲染；主表面使用视线厚度吸收代替实体式方向高光，厚处沉入阴影、薄处透出魂光，并以厘米级以内的连续轮廓呼吸和稳定雾化噪声打散规则外缘。轮廓层不再把整个人体从原点同比放大，而是沿局部表面法线做 8–26mm 的分层推出，因此手、脚、肩与头部不会出现比例相关的复制壳错位。向上流动的内层魂流与连续孔隙进一步削弱透明塑料感，同时保留完整人体轮廓。V5.11 进一步加入多尺度体积空洞、内部魂流和破碎外壳覆盖，并降低均匀亮度与壳层不透明度，使近景从发光塑料转向有暗部、有纵深的模糊灵体。120/48/0 分档 GPU 魂屑改为稀疏的纵向上升细迹，不再形成密集圆点；高／中画质另有无硬圆环的贴地魂雾，使脚部与场景形成低密度能量交换。白灵另有正常混合的深色对比轮廓，在明亮场景中也不会融入背景。
7. V6.8 赛博人物投影已实现青／紫双预设、细扫描带、数据块、地面投影源、96/40/0 分档空间信号符号，以及每 3.2 秒一次、持续 120ms 且可完全恢复的局部相位事件；最高画质增加独立相位余像层，在平时保留微弱载波断层、相位事件中生成短距离色散错位。V6.8 将宽扫描条收敛为高频横向微载波与稀疏纵向列载波，并加入随时间变化的信号完整度缺口和轻微色散，让近景不再像贴在人体上的宽条纹。赛博轮廓与相位余像同样改为毫米级法线偏移，避免近景出现同比放大的人体复制层。投影源作为地面与人体之间的独立 decal 渲染，不再被预览地板遮挡，身体周围的十字数据包用于表现低信号传输。
