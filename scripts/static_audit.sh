...
grep -q "保存虚像警告：本地写入失败" Bridge/Views/ScanARView.swift || fail "scan save must surface local persistence failures"
...
grep -q "保存放置警告：本地写入失败" Bridge/Views/PlaceARView.swift || fail "place save must surface local persistence failures"
...
grep -q "无效放置或相关评论可能恢复" Bridge/Services/LocalStore.swift || fail "invalid placement cleanup must warn when local persistence fails"
...
grep -q "评论警告：本地写入失败" Bridge/Views/Components/CommentThreadView.swift || fail "comment thread must surface local comment persistence failures"
grep -q "回复警告：本地写入失败" Bridge/Views/Components/CommentThreadView.swift || fail "comment thread must surface local reply persistence failures"
...
grep -q "删除放置警告：本地写入失败" Bridge/Views/MyPlacementsView.swift || fail "placement deletion must surface local persistence failures"
grep -q "删除虚像" Bridge/Views/AvatarsListView.swift || fail "avatar list deletion must be recorded in diagnostics"
grep -q "删除虚像警告：本地写入失败" Bridge/Views/AvatarsListView.swift || fail "avatar list deletion must surface local persistence failures"
grep -q "删除虚像" Bridge/Views/AvatarDetailView.swift || fail "avatar detail deletion must be recorded in diagnostics"
grep -q "删除虚像警告：本地写入失败" Bridge/Views/AvatarDetailView.swift || fail "avatar detail deletion must surface local persistence failures"
grep -q "删除评论警告：本地写入失败" Bridge/Views/Components/CommentThreadView.swift || fail "comment deletion must surface local persistence failures"
