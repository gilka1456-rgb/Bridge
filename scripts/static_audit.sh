...
grep -q "保存放置取消：页面已离开或预览锚点已变化" Bridge/Views/PlaceARView.swift || fail "place save must cancel stale async world map saves"
grep -q "保存放置取消：页面已离开，忽略错误提示" Bridge/Views/PlaceARView.swift || fail "place save must not surface async save errors after leaving the page"
