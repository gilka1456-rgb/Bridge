...
[[ -x scripts/device_preflight.sh ]] || fail "scripts/device_preflight.sh must exist and be executable"
...
grep -q "fail_xcode_first_launch" scripts/preflight.sh || fail "preflight must distinguish missing Xcode first-launch components"
grep -q "sudo xcodebuild -runFirstLaunch" scripts/preflight.sh || fail "preflight must show the Xcode first-launch component install command"
grep -q "fail_ios_platform_support" scripts/preflight.sh || fail "preflight must distinguish missing iOS platform support"
grep -q "xcodebuild -downloadPlatform iOS" scripts/preflight.sh || fail "preflight must show the iOS platform support download command"
grep -q "security find-identity -v -p codesigning" scripts/device_preflight.sh || fail "device preflight must check code signing identities"
grep -q "DEVELOPMENT_TEAM is empty" scripts/device_preflight.sh || fail "device preflight must check Xcode Team"
grep -q "xcrun xctrace list devices" scripts/device_preflight.sh || fail "device preflight must check physical iPhone visibility"
...
grep -q "WorldMap 文件名无效、WorldMap 解码失败、WorldMap 缺少目标锚点或 transform 异常" Bridge/Views/DiagnosticsView.swift || fail "invalid placement cleanup footer must match current cleanup criteria"
...
grep -q "WorldMap 解码失败" Bridge/Views/DiagnosticsView.swift || fail "diagnostics view invalid placement cleanup footer must mention world map decode failures"
...
grep -q "WorldMap 解码失败" Bridge/Services/LocalStore.swift || fail "invalid placement cleanup must count world map decode failures separately"
...
grep -q "worldMapAnchorCache" Bridge/Services/LocalStore.swift || fail "invalid placement cleanup must cache decoded world map anchor identifiers"
grep -q "WorldMapAnchorMembership" Bridge/Services/LocalStore.swift || fail "invalid placement cleanup must classify anchor membership separately from decode failures"
...
grep -q "WorldMap 解码失败" docs/iphone_mvp_test_plan.md || fail "iPhone MVP test plan must cover corrupt world map cleanup separately"
...
grep -q "xcode_install_state" scripts/collect_diagnostics.sh || fail "diagnostics bundle must include Xcode install state evidence"
grep -q "device_preflight" scripts/collect_diagnostics.sh || fail "diagnostics bundle must include device signing/device readiness evidence"
grep -q "Xcode.appdownload" scripts/collect_diagnostics.sh || fail "diagnostics bundle must capture App Store Xcode placeholder state"
...
grep -q "xcode_install_state.txt" scripts/collect_diagnostics.sh || fail "diagnostics README must explain Xcode install state evidence"
grep -q "git_revision.txt" docs/iphone_device_setup.md || fail "device setup docs must explain git revision evidence"
grep -q "xcode_install_state.txt" docs/iphone_device_setup.md || fail "device setup docs must explain Xcode install state evidence"
...
grep -q "sudo xcodebuild -license accept" docs/iphone_device_setup.md || fail "device setup docs must explain Xcode license acceptance"
grep -q "sudo xcodebuild -runFirstLaunch" docs/iphone_device_setup.md || fail "device setup docs must explain Xcode first-launch component installation"
grep -q "xcodebuild -downloadPlatform iOS" docs/iphone_device_setup.md || fail "device setup docs must explain iOS platform support installation"
grep -q "./scripts/device_preflight.sh" docs/iphone_device_setup.md || fail "device setup docs must explain device preflight"
grep -q "DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer ./scripts/preflight.sh" docs/iphone_device_setup.md || fail "device setup docs must explain temporary DEVELOPER_DIR preflight"
grep -q 'Xcode app is missing or stuck as `.appdownload`' docs/iphone_device_setup.md || fail "device setup docs must distinguish incomplete Xcode download"
grep -q "Xcode license not accepted" docs/iphone_device_setup.md || fail "device setup docs must distinguish unaccepted Xcode license"
grep -q "Xcode first-launch components not installed" docs/iphone_device_setup.md || fail "device setup docs must distinguish missing first-launch components"
grep -q "iOS platform support not installed" docs/iphone_device_setup.md || fail "device setup docs must distinguish missing iOS platform support"
grep -q "iPhoneOS SDK unavailable" docs/iphone_device_setup.md || fail "device setup docs must distinguish unavailable iPhoneOS SDK"
grep -q "Xcode cannot install app on iPhone" docs/iphone_device_setup.md || fail "device setup docs must distinguish device install/signing failures"
