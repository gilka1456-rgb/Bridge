...
grep -q "local snapshot load warning" docs/iphone_mvp_test_plan.md || fail "iPhone MVP test plan must cover local snapshot load warnings"
...
grep -q "opens on Scan" docs/iphone_mvp_test_plan.md || fail "iPhone MVP test plan must require empty first-run scan route"
